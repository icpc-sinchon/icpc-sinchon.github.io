...
const Suapc = ({ seasonData_, seasonList_ }) => {
    ...
    const fetchSeasonData = async (season) => {
        try {
            let response = await fetch(`http://localhost:4789/history/suapc/${season}.json`)
...
Suapc.getInitialProps = async ({ store }) => {
    ...
    let response0 = await fetch(`http://localhost:4789/history/suapc/${process.env.NEXT_PUBLIC_CURRENT_SUAPC_SEASON}.json`)
    let data0 = await response0.json();
    let response1 = await fetch(`http://localhost:4789/history/suapc/list.json`)
