...
Suapc.getInitialProps = async ({ store }) => {
    let response0 = await fetch(`${process.env.NEXT_LOCALHOST}/history/suapc/${process.env.NEXT_PUBLIC_CURRENT_SUAPC_SEASON}.json`)
    let data0 = await response0.json();
    let response1 = await fetch(`${process.env.NEXT_LOCALHOST}/history/suapc/list.json`)
