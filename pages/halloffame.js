...
const HallOfFame = ({ seasonList_, seasonData_ }) => {
    ...
    const fetchSeasonData = async (season) => {
        try {
            let response = await fetch(`http://localhost:4789/history/halloffame/${season}.json`)
...
HallOfFame.getInitialProps = async ({ store }) => {
    let response0 = await fetch(`http://localhost:4789/history/halloffame/${process.env.NEXT_PUBLIC_CURRENT_HALLOFFAME_SEASON}.json`)
    let data0 = await response0.json();
    let response1 = await fetch(`http://localhost:4789/history/halloffame/list.json`)
